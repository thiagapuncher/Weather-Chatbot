...
def extract_location(query):
    ...
    # common prepositions and words to exclude
    prepositions = ['in', 'at', 'on', 'near', 'around', 'by', 'for']
    excluded_words = ['the', 'and', 'what', 'like', 'can', 'do', 'is', 'there']

    # look for preposition in the query to determine the city
    for i, word in enumerate(words):
        if word in prepositions and i + 1 < len(words):
            # Return only the next word after preposition which is likely the city
            potential_city = words[i + 1]
            if potential_city not in excluded_words:
                return potential_city.title()

    # if no preposition found, return  last word that is not a preposition 
    for word in reversed(words):
        if word not in prepositions + excluded_words:
            return word.title()
...
def get_activities_suggestions(city, activity_type):
    ...
    logging.debug(f"Geocoding Response for {city}: {geo_response}")
